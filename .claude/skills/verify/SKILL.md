...
## Gotchas
- Every `.sql` file needs a `-- Tags: db1` header or the run halts. No `-- Checksum:` header is needed — checksums are computed from file content at apply/plan time (`ScriptParser.ComputeChecksum`).
